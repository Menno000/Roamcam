...
<div class="grid">
  ...
  <div class="card span2" data-tab="none" style="display:none"><h2>Opnames <span id="recCount" class="muted" style="font-weight:400"></span></h2><div class="body">
...
async function loadClips(){try{const list=await jget('/clips.json');
  ...
  $('clipCount').textContent=(list.length||0)+' '+tt('clips','clips')+' · '+fmtBytes(tot);
  if(!list.length){$('clipGallery').innerHTML='<span class="muted">'+tt('noClips','nog geen video-clips — start de opname in Instellingen')+'</span>';return;}
